...
- **QR endpoint is always public (`GET /api/v1/urls/{code}/qr`):** No auth regardless of the `[auth] api_key` setting — deliberate. QR is a visual encoding of the redirect URL, which is already public (anyone can visit the short link). Making QR auth-only would be security theatre and would break the landing page widget. Self-hosters who need a fully private instance (QR + create both require auth) must manually move the `/urls/{code}/qr` `r.Group` block into the protected group in `internal/api/router.go`. A config toggle (`[public] enabled = true/false`) is the planned improvement — tracked as D16 in `docs/DESIGN.md`.
