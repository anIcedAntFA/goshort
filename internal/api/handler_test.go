...
func TestGetQRCode(t *testing.T) {
	...
	// Create a URL to use in QR tests.
	body := mustMarshal(t, map[string]string{"url": "https://example.com"})
	rec := serve(t, deps.router, http.MethodPost, "/api/v1/urls", body)
	if rec.Code != http.StatusCreated {
	...
	tests := []struct {
		...
		{
			name:       "valid code default size",
			code:       code,
			query:      "",
			wantStatus: http.StatusOK,
		},
		{
			name:       "valid code explicit size 128",
			code:       code,
			query:      "?size=128",
			wantStatus: http.StatusOK,
		},
		{
			name:       "size 2000 clamped to 1024",
			code:       code,
			query:      "?size=2000",
			wantStatus: http.StatusOK,
		},
		{
			name:       "size 50 clamped to 128",
			code:       code,
			query:      "?size=50",
			wantStatus: http.StatusOK,
		},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			body := r.Body.Bytes()
			if len(body) < 8 {
				t.Fatalf("body too short (%d bytes) to contain PNG magic", len(body))
			}
			if !bytes.Equal(body[:8], pngMagic) {
				t.Errorf("body does not start with PNG magic bytes, got %x", body[:8])
			}
		})
	}
}
